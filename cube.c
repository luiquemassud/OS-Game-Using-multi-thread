...
void kill_wizards(struct cube *cube){

  /* Fill in */
  int i;
  for (i = 0;i<cube->teamA_size;i++){
    pthread_cancel(cube->teamA_wizards[i]->wiztid);
  }
  for (i = 0;i<cube->teamB_size;i++){
    pthread_cancel(cube->teamB_wizards[i]->wiztid);
  } 
  return;
}
  ...
  while (1){
    ...
    if (!strcmp(command, "exit")){
      ...
      if (cube->game_status == 1){
        ...
        for (i = 0; i < cube->teamA_size;i++){
          pthread_t tid;
          pthread_create(&tid, NULL, &wizard_func, cube->teamA_wizards[i]);
          // printf("\nIn Main, TID created: %u\n", tid);
        }
        for (i = 0; i < cube->teamB_size;i++){
          pthread_t tid;
          pthread_create(&tid, NULL, &wizard_func, cube->teamB_wizards[i]);
          // printf("\nIn Main, TID created: %u\n", tid);
        }
